...
<div class="page-container">
    <form action="" method="post">
        <div class="text-c">
            ...
            <span class="select-box inline">
                分类:
                <select name="managersId" class="select">
                    <option value="">请选择</option>
                    <c:forEach items="${requestScope.managersId}" var="listTemp" varStatus="stat">
                        <option value="${listTemp.id}"${requestScope.managersId == listTemp.id ? 'selected' : ''}>${listTemp.name}</option>
                    </c:forEach>
                </select>
                状态:
                <select name="status" class="select">
                    ...
                    <option value="1" ${requestScope.status == "1" ? 'select' : ''}>启用</option>
                    <option value="0" ${requestScope.status == "1" ? 'select' : ''}>禁用</option>
                </select>
            </span>
            发布时间：
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
            ...
            <a
                href="javascript:;" onclick="member_add('添加管理员','${rootPath}/back/users/managersInsert.mvc','','510')"
                        ...
                        <tr class="text-c">
                            ...
                            <!--<td>
                                <u style="cursor:pointer" class="text-primary"
                                   onclick="member_show('查看分类','${rootPath}/back/users/managersUpdate.mvc?id=${listTemp.id}','','','510')">
                                        <%--${listTemp.name}--%>

                                </u>
                            </td>-->
<%--                            <td>${listTemp.managers.username}</td>--%>
                            <td>${listTemp.username}</td>

                            <td>${listTemp.email}</td>
                            <td>${listTemp.password}</td>
                            <td>${listTemp.phone}</td>
                            <td>${listTemp.balance}</td>
                            <td>${listTemp.sex}</td>
                            ...
                            <td><fmt:formatDate value="${listTemp.lastLoginTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
...
<%--                                </a>--%>
                                <a
                                    title="编辑" href="javascript:;" onclick="member_edit('编辑','${rootPath}/back/users/managersUpdate.mvc?id=${listTemp.id}&operType=update','4','','510')"
...
<%--                                </a>--%>
            ...
            <form id="pageForm" action="${rootPath}/back/users/managersList.mvc" method="post">
                <!-- 搜索条件 -->
                <input type="hidden" name="keyword" value="${requestScope.keyword}">
                <input type="hidden" name="status" value="${requestScope.status}">
                <input type="hidden" name="st" value="${requestScope.st}">
                <input type="hidden" name="ed" value="${requestScope.ed}">
                <input type="hidden" name="managersId" value="${requestScope.managersId}">
...
<script type="text/javascript"  src="${rootPath}/common/resources/H-ui.admin/lib/datatables/1.10.15/jquery.dataTables.min.js"></script>
